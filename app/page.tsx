...
const whatIDo = [
  ...
  },
  {
    ...
    skills: ["Next.js", "TypeScript", "Tailwind CSS", "Framer Motion", "Shadcn", "React", "Claude Code"],
  },
  {
    title: "Growth",
...
const books = [
  { title: "Refactoring UI", author: "Adam Wathan & Steve Schoger", year: "2022" },
  { title: "How To Write Unmaintainable Code", author: "Roedy Green", year: "2021" },
  { title: "(Design) Sprint", author: "Jake Knapp", year: "2018" },
  { title: "The Design of Everyday Things", author: "Donald Norman", year: "2019" },
  { title: "Atomic Design", author: "Brad Frost", year: "2017" },
  { title: "Designing Interfaces", author: "Tidwell, Brewer & Valencia", year: "2016" },
      ...
      <div className="mx-auto overflow-hidden sm:overflow-visible sm:px-8 xl:max-w-8xl xl:px-8">
        <div className="grid grid-cols-1 items-start border-zinc-200 dark:border-zinc-800 sm:mt-8 sm:border-t xl:grid-cols-[34rem_minmax(0,1fr)]">
          ...
          <div className="relative border-b border-zinc-200 dark:border-zinc-800 px-8 pb-12 pt-8 sm:border-x sm:px-12 sm:py-16 xl:sticky xl:top-0 xl:border-r-0 xl:min-h-screen">
            ...
            <p className="mt-3 text-base text-zinc-500 dark:text-zinc-400">Design Engineer</p>

            {/* Meta */}
            <dl className="mt-4 flex flex-wrap gap-x-6 gap-y-1 text-sm text-zinc-400 dark:text-zinc-500">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <a href="mailto:remi.charreau@gmail.com" className="hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
            ...
            <div className="mt-5 flex gap-3">
              {[
                { label: "LinkedIn", href: "https://www.linkedin.com/in/remicharreau" },
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 px-3 py-1 text-xs text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                >
                  {label}
          ...
          <div className="relative px-8 pb-16 pt-12 border-zinc-200 dark:border-zinc-800 sm:mb-24 sm:border-x sm:border-b sm:px-12 sm:pt-16">
            ...
            <section className="mb-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                {whatIDo.map((item) => (
                  <div key={item.title}>
                    <p className="text-sm font-medium text-zinc-900 dark:text-white mb-2">{item.title}</p>
                    <p className="text-sm leading-6 text-zinc-500 dark:text-zinc-400 mb-3">{item.desc}</p>
                    <ul className="mb-4 space-y-1">
                      {item.tasks.map((task) => (
                        <li key={task} className="text-xs text-zinc-400 dark:text-zinc-500 flex items-start gap-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {item.skills.map((skill) => (
                        <span key={skill} className="inline-flex items-center rounded-full border border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 px-2.5 py-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                          {skill}
                        </span>
                      ))}
                    </div>
              ...
              <div className="space-y-7">
                {work.map((item) => (
                  <div key={item.name} className="flex gap-4">
                    <div className="shrink-0 size-10 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 overflow-hidden flex items-center justify-center">
                      <img src={item.logo} alt={item.name} width={40} height={40} className="object-contain" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-zinc-900 dark:text-white leading-tight">{item.name}</p>
                      <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5 tabular-nums">{item.period}</p>
                ...
                {courses.map((item) => (
                  <div key={item.title} className="flex items-baseline justify-between gap-4">
                    <div className="flex items-baseline gap-2 min-w-0">
                      <p className="text-sm text-zinc-900 dark:text-white shrink-0">{item.title}</p>
                      {item.author && (
                        <p className="text-xs text-zinc-400 dark:text-zinc-500 truncate">· {item.author}</p>
                      )}
                    </div>
                    <p className="text-xs text-zinc-400 dark:text-zinc-500 tabular-nums shrink-0">{item.year}</p>
                  </div>
                ))}
              </div>
            </section>
